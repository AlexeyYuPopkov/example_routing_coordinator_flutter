...
последующем. Наиболее очевидным решением видится вынести реализацию перехода из
кода экрана.
...
нет. Она есть у там, где приложение собрается из экранов. Сам-же экран, пусть включает абстрактный интерфейс такого перехода. Тогда типичный переход будет выглядить как-то так:
...
Саму реализацию этого интерфейса нужно держать как можно ближе к тому месту, где выполняется сама конфигурация переходов (или другими словами, откуда, так или иначе, был вызван конструктор текущего экрана).
